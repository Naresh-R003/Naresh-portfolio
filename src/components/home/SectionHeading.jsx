export default function SectionHeading({ kicker, title, children }) {
  return (
    <div className="gsap-reveal mx-auto mb-10 max-w-3xl text-center md:mb-14">
      ...
      <h2 className="mt-4 text-pretty text-3xl font-semibold md:text-5xl">
        <span className="gradient-heading">{title}</span>
      </h2>
      {children ? (
        <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-white/90 md:text-lg">{children}</p>
      ) : null}
    </div>
  );
}
